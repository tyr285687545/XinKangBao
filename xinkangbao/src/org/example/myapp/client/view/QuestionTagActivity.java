 ...
 * @created 2012-8-27
 */
public class QuestionTagActivity extends Activity {
	...
	private void initView(){
        ...
        lvQuestion.addFooterView(lvQuestion_footer);
        ...
        lvQuestion.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		//跳转到问答详情
        		//UIDefine.showQuestionDetail(view.getContext(), post.getId());
        	}        	
		});
        lvQuestion.setOnScrollListener(new AbsListView.OnScrollListener() {
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				if(scrollEnd && lvDataState==UIDefine.LISTVIEW_DATA_MORE)
				{
					...
					//褰撳墠pageIndex
					int pageIndex = lvQuestionSumData/org.example.myapp.common.AppContext.PAGE_SIZE;
					loadLvQuestionData(curTag, pageIndex, lvQuestionHandler, UIDefine.LISTVIEW_ACTION_SCROLL);
				}
			}
        ...
        lvQuestion.setOnRefreshListener(new PullToRefreshListView.OnRefreshListener() {
            public void onRefresh() {
            	loadLvQuestionData(curTag, 0, lvQuestionHandler, UIDefine.LISTVIEW_ACTION_REFRESH);
            }
        });			
	}
	...
	private void initData(){
		lvQuestionHandler = new Handler()
		{
			...
			{				
				if(msg.what >= 0){	
						...
						if(lvQuestionData.size() > 0){
							for(Post p1 : list.getPostlist()){
								...
								for(Post p2 : lvQuestionData){
									if(p1.getTopic_id() == p2.getTopic_id() && p1.getUid() == p2.getUid()){
										b = true;
										break;
									}
								}
					...
					((AppException)msg.obj).makeToast(QuestionTagActivity.this);
				}
				if(lvQuestionData.size()==0){
		...
		this.loadLvQuestionData(curTag, 0, lvQuestionHandler, UIDefine.LISTVIEW_ACTION_INIT);
    }
		...
		new Thread(){
			public void run() {
				...
				if(action == UIDefine.LISTVIEW_ACTION_REFRESH || action == UIDefine.LISTVIEW_ACTION_SCROLL)
					isRefresh = true;
				ReturnObj ret =  MainActivity.client_in_strict_mode.get_post_list(MainActivity.myUser.getId());
				Log.i("wangbo debug", ret.getOrg_str());
				if (ret.getRet_code() == 0) {
					PostList list = PostList.parse(ret.getOrg_str());
					msg.what = list.getPageSize();
					msg.obj = list;
					Log.i("wangbo debug", "new list: " + Integer.toString(list.getPostlist().size()));
				}
